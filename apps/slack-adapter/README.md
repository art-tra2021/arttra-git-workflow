...
Issue作成時点の担当者は最初の概要に含め、同じ担当者に対する初期`assigned` eventを独立した返信にしない。
初期担当者はIssueごとの永続stateで一度だけ消費し、処理時刻による時間窓では判定しない。
作成後の`unassigned`と再assignmentは通常どおり通知する。
...
Task作成eventより続報が先に届いても親WorkまたはBusinessの概要とTask概要を順に確保してから続報を送り、channel直下へ平投稿しない。PR作成・レビュー依頼の別経路でも同じTask概要intentを使う。親を解決できないTaskは平投稿へfallbackしない。
